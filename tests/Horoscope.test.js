...
describe('Houses', () => {
  ...
  test('Regiomontanus', () => {
    expect(new Horoscope({origin: defaultOrigin, houseSystem: 'regiomontanus'}).Houses.map(c => c.ChartPosition.StartPosition.Ecliptic.DecimalDegrees)).toEqual([ 169.4304, 195.1442, 224.3037, 258.4576, 293.2347, 323.3454, 349.4304, 15.1442, 44.3037, 78.4576, 113.2347, 143.3454 ])
  })

  test('Topocentric', () => {
    expect(new Horoscope({origin: defaultOrigin, houseSystem: 'topocentric'}).Houses.map(c => c.ChartPosition.StartPosition.Ecliptic.DecimalDegrees)).toEqual([169.4304,195.8759,226.0562,258.4576,290.9246,321.6638,349.4304,15.8759,46.0562,78.4576,110.9246,141.6638])
  })
...
describe('ZodiacCusps', () => {
  ...
  describe('tropical', () => {
    test('origin 1', () => {
      ...
      expect(horoscope.ZodiacCusps.map(c => c.ChartPosition.Horizon.DecimalDegrees)).toEqual([169.4304,139.4304,109.4304,79.4304,49.4304,19.4304,349.4304,319.4304,289.4304,259.4304,229.4304,199.4304,])

      expect(horoscope.ZodiacCusps.map(c => c.ChartPosition.Ecliptic.DecimalDegrees)).toEqual([ 0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330 ])
  ...
  describe('sidereal', () => {
    test('origin 1', () => {
      ...
      expect(horoscope.ZodiacCusps.map(c => c.ChartPosition.Horizon.DecimalDegrees)).toEqual([ 145.3304,115.3304,85.3304,55.3304,25.3304,355.3304,325.3304,295.3304,265.3304,235.3304,205.3304,175.3304,])
...
describe('CelestialBodies', () => {
  describe('tropical', () => {
    test('origin 1', () => {
      ...
      expect(horoscope.CelestialBodies.map(b => b.ChartPosition.Horizon.DecimalDegrees)).toEqual(
        [ 52.0027,
          193.0866,
          50.1977,
          58.8471,
